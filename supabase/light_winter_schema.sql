create table if not exists public.lwr_organizations (
  id text primary key,
  name text not null,
  fiscal_mode boolean not null default false,
  phone text not null default '',
  address text not null default '',
  registered_name text not null default '',
  tin text not null default '',
  vat_number text not null default '',
  zimra_device_id text not null default '',
  fiscal_serial_number text not null default '',
  fiscal_qr_url text not null default '',
...
create table if not exists public.lwr_branches (
  id text primary key,
...
create table if not exists public.lwr_devices (
  id bigint generated by default as identity primary key,
  device_uid text not null unique,
  organization_id text not null references public.lwr_organizations(id) on delete cascade,
  branch_id text not null references public.lwr_branches(id) on delete restrict,
  device_name text not null default 'Light Winter POS Device',
...
create table if not exists public.lwr_activation_codes (
  code text primary key,
  organization_id text not null references public.lwr_organizations(id) on delete cascade,
  branch_id text not null references public.lwr_branches(id) on delete cascade,
  active boolean not null default true,
  created_at timestamptz not null default now()
);
...
create table if not exists public.lwr_users (
  id text primary key,
  organization_id text not null references public.lwr_organizations(id) on delete cascade,
  name text not null,
...
create table if not exists public.lwr_products (
  id text primary key,
  organization_id text not null references public.lwr_organizations(id) on delete cascade,
  name text not null,
  sku text not null,
...
create table if not exists public.lwr_branch_stock (
  branch_id text not null references public.lwr_branches(id) on delete cascade,
  product_id text not null references public.lwr_products(id) on delete cascade,
  quantity integer not null default 0,
  updated_at timestamptz not null default now(),
  primary key (branch_id, product_id)
);

create table if not exists public.lwr_customers (
  id text primary key,
  organization_id text not null references public.lwr_organizations(id) on delete cascade,
  name text not null,
  phone text not null default '',
...
create table if not exists public.lwr_sales (
  id text primary key,
  organization_id text not null references public.lwr_organizations(id) on delete cascade,
  branch_id text not null references public.lwr_branches(id) on delete restrict,
...
create table if not exists public.lwr_sale_lines (
  id text primary key,
...
create table if not exists public.lwr_license_tokens (
  ...
  duration_mode text not null check (duration_mode in ('days', 'minutes')),
...
create table if not exists public.lwr_licenses (
  id bigint generated by default as identity primary key,
  device_uid text not null,
  token text not null,
...
create table if not exists public.lwr_audit_events (
  id bigint generated by default as identity primary key,
...
alter table public.lwr_organizations enable row level security;
alter table public.lwr_branches enable row level security;
alter table public.lwr_devices enable row level security;
...
alter table public.lwr_sale_lines enable row level security;
...
alter table public.lwr_license_tokens enable row level security;
alter table public.lwr_owner_reset_tokens enable row level security;
alter table public.lwr_licenses enable row level security;
...
begin
  ...
  loop
    execute format('drop policy if exists lwr_anon_all on public.%I', table_name);
    execute format('create policy lwr_anon_all on public.%I for all to anon using (true) with check (true)', table_name);
  end loop;
end $$;
...
create index if not exists idx_lwr_branches_org on public.lwr_branches(organization_id);
create index if not exists idx_lwr_users_org on public.lwr_users(organization_id);
create index if not exists idx_lwr_products_org on public.lwr_products(organization_id);
create index if not exists idx_lwr_sales_org_branch on public.lwr_sales(organization_id, branch_id);
...
begin
  ...
  values (org_id, trim(p_shop_name), coalesce(p_fiscal_mode, false), coalesce(p_main_branch->>'phone', ''), coalesce(p_main_branch->>'address', ''), recovery, true);

  insert into public.lwr_branches(id, organization_id, name, phone, address, active)
  values (main_branch_id, org_id, coalesce(nullif(p_main_branch->>'name', ''), 'Main Branch'), coalesce(p_main_branch->>'phone', ''), coalesce(p_main_branch->>'address', ''), true);
  ...
  for branch_item in select value from jsonb_array_elements(coalesce(p_branches, '[]'::jsonb)) loop
    ...
    insert into public.lwr_branches(id, organization_id, name, phone, address, active)
    values (branch_id, org_id, coalesce(nullif(branch_item->>'name', ''), 'Branch'), coalesce(branch_item->>'phone', ''), coalesce(branch_item->>'address', ''), true);
    insert into public.lwr_activation_codes(code, organization_id, branch_id, active)
  ...
  on conflict (device_uid) do update set organization_id = excluded.organization_id, branch_id = excluded.branch_id, active = true, last_seen_at = now();

  insert into public.lwr_users(id, organization_id, name, username, role, pin_plain, permissions, active)
  values (owner_id, org_id, coalesce(nullif(p_owner->>'name', ''), 'Owner'), coalesce(nullif(p_owner->>'username', ''), 'owner'), coalesce(nullif(p_owner->>'role', ''), 'owner'), coalesce(nullif(p_owner->>'pin_plain', ''), '0000'), coalesce(p_owner->'permissions', '[]'::jsonb), true);
